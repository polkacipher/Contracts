...
contract Manager is Context, Ownable {
    ...
    function setFaction(string memory name, address owner, uint256 supply) public virtual onlyOwner {
        _initiateFaction(name,owner,supply);
    }


    function _initiateFaction(string memory name, address owner, uint256 supply) internal virtual {
        _factions[index]=Faction(name,owner,supply);
        index++;
    }

    function getFaction(uint8 _index) public view virtual returns (Faction memory){
